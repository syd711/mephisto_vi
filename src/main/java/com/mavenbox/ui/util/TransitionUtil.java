...
public class TransitionUtil {
  ...
  public static ScaleTransition createScaler(Node node, double factor) {
    ScaleTransition scaleTransition = new ScaleTransition(Duration.millis(FADER_DEFAULT), node);
  ...
  private static void applyDefaults(Node node, Transition transition) {
    ...
    if(!node.getCacheHint().equals(CacheHint.QUALITY)) {
      node.setCache(true);
